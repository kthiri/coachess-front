...
export class ControlsComponent implements OnInit {
  ...
  public addToCart(product:Product){
    ...
    if(currentProduct){
      if((currentProduct.cartCount + this.count) <= this.product.quantite){
        product.cartCount = currentProduct.cartCount + this.count;
      }
      else{
        this.snackBar.open('You can not add more items than available. In stock ' + this.product.quantite + ' items and you already added ' + currentProduct.cartCount + ' item to your cart', '×', { panelClass: 'error', verticalPosition: 'top', duration: 5000 });
        return false;
      }
    }
    else{
